...
export class GeminiAdapter implements ProviderAdapter {
  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    const { url, headers, body, toolCallIdMap } = translateRequest(request, this.apiKey, false);
  ...
  async* stream(request: LLMRequest): AsyncIterable<StreamEvent> {
    const { url, headers, body, toolCallIdMap } = translateRequest(request, this.apiKey, true);
