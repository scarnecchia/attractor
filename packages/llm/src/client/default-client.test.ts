...
describe('Default Client', () => {
  ...
  describe('AC1.5: Lazy initialization from environment', () => {
    it('should create client from env on first call to getDefaultClient()', () => {
      ...
      expect(mockAdapterFactory).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'test-key' }));
    });
  ...
  describe('Reset for testing', () => {
    ...
    it('should re-initialize from env after resetDefaultClient()', () => {
      ...
      expect(client1).toBeInstanceOf(Client);
      expect(mockAdapterFactory1).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'key1' }));
      ...
      expect(client1).not.toBe(client2);
      expect(mockAdapterFactory2).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'key2' }));
    });
  });
});
